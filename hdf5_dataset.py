...
class HDF5Dataset(Dataset):
    """A class to create a PyTorch dataset using HDF5 files.
    
    Parameters
    ----------
    file_path : str
        Path to folder containing the dataset (one or multiple HDF5 files).
    recursive : bool
        If True, searches for h5 in subdirectories
    load_data : bool
        If True, loads all the data immediately into RAM. Use this if the
        dataset fits into memory. Otherwise, leave this at false and the
        data will load lazily.
    data_cache_size : int, optional
        Number of HDF5 files that can be cached in the cache, by default 3.
    transform : func
        PyTorch transform to apply to every data instance, by default None.
    """
